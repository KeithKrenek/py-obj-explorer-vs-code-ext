...
let objectExplorerPanel: vscode.WebviewPanel | undefined;
...
export function activate(context: vscode.ExtensionContext) {
    ...
    const disposable = vscode.commands.registerCommand(
        'python-object-explorer.explore',
        async () => {
...
async function exploreSelectedVariable(context: vscode.ExtensionContext) {
...
async function executeAndIntrospect(
    filePath: string, 
    lineNumber: number, 
    variableName: string,
...
function createIntrospectionScript(): string {
    return `import sys
import json
import traceback
...
async function showObjectExplorer(data: ObjectNode, context: vscode.ExtensionContext) {
    if (objectExplorerPanel) {
        // Reuse existing panel
        objectExplorerPanel.reveal();
    } else {
        // Create new panel
        objectExplorerPanel = vscode.window.createWebviewPanel(
            'pythonObjectExplorer',
        ...
        objectExplorerPanel.webview.onDidReceiveMessage(
            message => {
